...
- **Ne jamais casser les données persistées** : clés localStorage `<jeu>-score-v1`, format `{players:[{nom,d}], cur, started, totals, ts, exts?}` (`ts` = dernière utilisation, ordonne le menu de l'accueil), et `scores-history-v1`, tableau d'entrées `{g, t, players:[{nom,total}]}` (historique des parties, écrit par `common.js` au reset). Des parties réelles sont en cours sur téléphone. Toute évolution du format doit relire l'ancien (voir `fixup`/`restoreExtra` dans `common.js`).
