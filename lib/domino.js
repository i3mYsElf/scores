...
(function(){
  ...
  const domsHtml = domaines => domaines.map((m,i)=>`
    <div class="row">
      <input class="num num-s" data-dc="${i}" type="number" min="0" inputmode="numeric" value="${+m.c||0}" aria-label="cases du domaine">
      <span style="color:var(--muted);font-size:13px">cases ×</span>
      <input class="num num-s" data-dk="${i}" type="number" min="0" inputmode="numeric" value="${+m.k||0}" aria-label="couronnes du domaine">
      <span class="val rowtot" data-dtot="${i}">0</span>
