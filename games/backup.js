/* games/backup.js — aperçus « Partie en cours » et export/import des sauvegardes.
   Logique pure, sans DOM : le storage est passé en paramètre (localStorage dans
   les pages, un stub en test). Ce n'est pas un jeu : pas de blank/score. */
(function(){
  ...
  function previewLabel(s){
    if(!(s && s.started && Array.isArray(s.players) && Array.isArray(s.totals) && s.totals.some(t => t > 0))) return null;
